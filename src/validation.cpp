...
#if defined(NDEBUG)
# error "Bitclassic Coin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bitclassic Coin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BICC additional fees, %d delta bytes\n",
...
static bool IsUAHFenabled(const Consensus::Params& consensusparams, int nHeight) {
    return nHeight >= consensusparams.BICCForkHeight;
}
...
static bool XCatchBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, 
CBlockIndex** ppindex)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
...
int GetRightBestHeader(const std::vector<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex)
{
    ...
    for (i = 0; i < (int)headers.size(); ++i) {
        ...
        if (!XCatchBlockHeader(headers[i], state, chainparams, &pindex)) {
            break;
        }
    }
    return i;
