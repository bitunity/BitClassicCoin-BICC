...
bool VerifyWallets()
{
    ...
    for (const std::string& walletFile : gArgs.GetArgs("-wallet")) {
        ...
        if (!wallet_paths.insert(wallet_path).second) {//set会自动排序，元素不能重复，所以如果存在，就会插入失败，second表示是否插入成功
            return InitError(strprintf(_("Error loading wallet %s. Duplicate -wallet filename specified."), walletFile));
        }

        std::string strError;
        if (!CWalletDB::VerifyEnvironment(walletFile, GetDataDir().string(), strError)) {//检查berkerley数据库配置是否正确
...
bool OpenWallets()
{
    if (gArgs.GetBoolArg("-disablewallet", DEFAULT_DISABLE_WALLET)) {//默认配置是不启用钱包
