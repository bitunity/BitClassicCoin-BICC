...
bool CWallet::SetHDMasterKey(const CPubKey& pubkey)
{
    ...
    newHdChain.masterKeyID = pubkey.GetID();//生成比特币地址
    SetHDChain(newHdChain, false);//保存结果
...
CBlockIndex* CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    CBlockIndex* ret = nullptr;
    {
        ...
        double dProgressStart = GuessVerificationProgress(chainParams.TxData(), pindex);//开始区块和结束区块用来计算scan进度
        double dProgressTip = GuessVerificationProgress(chainParams.TxData(), chainActive.Tip());
        while (pindex && !fAbortRescan)
        {
            if (pindex->nHeight % 100 == 0 && dProgressTip - dProgressStart > 0.0)//计算scan进度
            ...
            CBlock block;
            if (ReadBlockFromDisk(block, pindex, Params().GetConsensus())) {//根据pindex读取一个区块的数据放到block中
                for (size_t posInBlock = 0; posInBlock < block.vtx.size(); ++posInBlock) {
                    AddToWalletIfInvolvingMe(block.vtx[posInBlock], pindex, posInBlock, fUpdate);//遍历区块的交易数据
                }
            } else {
                ret = pindex;
            }
            pindex = chainActive.Next(pindex);//获取下一个需要scan的区块index
        }
...
CAmount CWallet::GetAvailableBalance(const CCoinControl* coinControl) const//计算钱包的余额
{
    LOCK2(cs_main, cs_wallet);
...
void CWallet::AvailableCoins(std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t &nMaximumCount, const int &nMinDepth, const int &nMaxDepth) const
{
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (nDepth == 0 && !pcoin->InMempool())//depth=0表示:in memory pool, waiting to be included in a block，再判断是否在内存池，发现不在内存池，就有问题了
                continue;
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (gArgs.GetBoolArg("-zapwallettxes", false)) {//Delete all wallet transactions and only recover those parts of the  blockchain through -rescan on startup (1 = keep tx meta data e.g.  account owner and payment request information, 2 = drop tx meta  data
        uiInterface.InitMessage(_("Zapping all transactions from wallet..."));

        std::unique_ptr<CWalletDBWrapper> dbw(new CWalletDBWrapper(&bitdb, walletFile));//钱包数据存储在wallet.dat中，使用berkerley数据库
        std::unique_ptr<CWallet> tempWallet(new CWallet(std::move(dbw)));
        DBErrors nZapWalletRet = tempWallet->ZapWalletTx(vWtx);//通过CWalletDB类对象的ZapWalletTx()清除交易
    ...
    CWallet *walletInstance = new CWallet(std::move(dbw));//新建一个钱包对象
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun))//通过判断用户是否使用upgradewallet命令进行钱包升级，具体逻辑是获取用户使用upgradewallet的版本参数也就是版本号进行判断，如果nMaxVersion为0（也就是没有输入要升级到的版本号，则使用CLIENT_VERSION系统定义的版本号。通过调用SetMinVersion进行升级
    {
    ...
    if (fFirstRun)
    {
        ...
        // generate a new master key确定性钱包，生成一个新的种子密钥
    ...
    RegisterValidationInterface(walletInstance);//注册钱包事件，比如交易更新等

    // Try to top up keypool. No-op if the wallet is locked.
    walletInstance->TopUpKeyPool();

    CBlockIndex *pindexRescan = chainActive.Genesis();//chainActive是存储的区块的位置信息链，通过遍历位置链来获取相应的区块数据及交易数据。
    if (!gArgs.GetBoolArg("-rescan", false))//如果用户输入了rescan参数，则从创世块开始获取需要重新更新钱包数据的位置
    {
    ...
    if (chainActive.Tip() && chainActive.Tip() != pindexRescan)
    {
        ...
        nStart = GetTimeMillis();//从1970-1-1到当前时间的毫秒数
        walletInstance->ScanForWalletTransactions(pindexRescan, true);//开始扫描钱包中的交易
