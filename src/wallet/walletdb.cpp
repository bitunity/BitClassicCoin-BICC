             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        else if (strType == "tx")
        {
            ...
            if (!(CheckTransaction(wtx, state) && (wtx.GetHash() == hash) && state.IsValid()))//检查交易数据是否正确
                return false;
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    try {
        ...
        // Get cursor
        Dbc* pcursor = batch.GetCursor();//游标（cursor）是数据库应用中常见概念，其本质上就是一个关于特定记录的遍历器。注意到DB支持多重记录（duplicate records），即多条记录有相同关键字，在对多重记录的处理中，使用游标是最容易的方式
        if (!pcursor)
        {
...
DBErrors CWalletDB::ZapWalletTx(std::vector<CWalletTx>& vWtx)
{
    ...
    std::vector<uint256> vTxHash;
    DBErrors err = FindWalletTx(vTxHash, vWtx);//将钱包中的"tx"交易数据保存到vTxHash中，然后erase
    if (err != DB_LOAD_OK)
        return err;
